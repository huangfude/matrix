...
use matrix_router_rs::{
    app::server,
    conf::{RoutingModeExt, MatrixConfig},
    version,
};
...
#[derive(Parser, Debug)]
#[command(name = "matrix-router", alias = "matrix")]
#[command(about = "Matrix Router - High-performance inference gateway")]
#[command(args_conflicts_with_subcommands = true)]
#[command(long_about = r#"
Matrix Router - Rust-based inference gateway

Usage:
  matrix launch [OPTIONS]     Launch router (short command)
  matrix-router [OPTIONS]  Launch router (full name)
  

Examples:
  # Regular mode
  matrix-router --worker-urls http://worker1:8000 http://worker2:8000

  # PD disaggregated mode
  matrix-router --pd-disaggregation \
...
struct CliArgs {
    ...
    // vllm ascend proxy mode
    #[arg(long, num_args = 0..)]
    prefiller_hosts: Vec<String>,

    #[arg(long, num_args = 0..)]
    prefiller_ports: Vec<u16>,

    #[arg(long, num_args = 0..)]
    decoder_hosts: Vec<String>,

    #[arg(long, num_args = 0..)]
    decoder_ports: Vec<u16>,
    ...
    //  设置Static Models，直接手动指定worker模型
    #[arg(long, num_args = 0..)]
...
impl CliArgs {
    ...
    fn to_matrix_config(&self, prefill_urls: Vec<(String, Option<u16>)>) -> MatrixConfig {
        let mut mode_ext = RoutingModeExt::TBD;
        //  pd分离模式且backend为vllm时，使用VllmPrefillDecode模式
        if self.pd_disaggregation {
            ...
            let prefill_policy = self.prefill_policy.as_ref().map(|p| self.parse_policy(p));
            let decode_policy = self.decode_policy.as_ref().map(|p| self.parse_policy(p));

            mode_ext = match self.backend {
                Some(Backend::Vllm) => {
                    ...
                    if use_static_urls && use_discovery {
                        eprintln!(
                            "ℹ️  INFO: Using hybrid mode - static URLs as fallback, service discovery for dynamic workers."
                        );
                        eprintln!("   Prefill URLs: {:?}", prefill_urls);
                        eprintln!("   Decode URLs: {:?}", decode_urls);
                    ...
                    RoutingModeExt::VllmPrefillDecode {
                        ...
                        prefill_policy: prefill_policy.clone(),
                ...
                _ => RoutingModeExt::TBD,
            };
        }

        MatrixConfig {
            routing_mode_ext: mode_ext,
            static_models: Some(self.static_models.clone()),
            data_plane_api_keys: Some(self.data_plane_api_keys.clone()),
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut prefill_urls = parse_prefill_args();
    ...
    // 扩展prefill_urls和decode_urls参数
    prefill_urls.extend(cli_args.prefill_urls.iter().map(|url| (url.clone(), None)));
    cli_args.decode.extend(cli_args.decode_urls.clone());

    if !cli_args.prefiller_hosts.is_empty() && !cli_args.decoder_hosts.is_empty() {
        // 扩展vllm ascend参数
        // prefiller urls参数
        let p_urls: Vec<(String, Option<u16>)> = cli_args
            .prefiller_hosts
            .iter()
            .zip(cli_args.prefiller_ports.iter())
            .map(|(host, port)| (format!("http://{}:{}", host, port), None))
            .collect();
        prefill_urls.extend(p_urls);
        // decoder urls参数
        let d_urls: Vec<String> = cli_args
            .decoder_hosts
            .iter()
            .zip(cli_args.decoder_ports.iter())
            .map(|(host, port)| format!("http://{}:{}", host, port))
            .collect();
        cli_args.decode.extend(d_urls);
    }

    if !cli_args.enable_igw {
        if !prefill_urls.is_empty() {
            cli_args.pd_disaggregation = true;
        }
        // Automatically enable IGW mode when service discovery is turned on
        if cli_args.service_discovery {
    ...
    let router_config = cli_args.to_router_config(prefill_urls.clone())?;
    router_config.validate()?;

    let server_config = cli_args.to_server_config(router_config)?;
    let matrix_config = cli_args.to_matrix_config(prefill_urls);
    let runtime = tokio::runtime::Runtime::new()?;
    runtime.block_on(async move { server::startup(server_config, matrix_config).await })?;
    if is_otel_enabled() {
        shutdown_otel();
    }
    Ok(())
