...
from matrix_cli.matrix_cli_rs import get_available_reasoning_parsers, get_available_tool_call_parsers
...
class RouterArgs:
    ...
    static_models: list[str] = dataclasses.field(default_factory=list)
    data_plane_api_keys: list[str] = dataclasses.field(default_factory=list)
    ...
    def add_cli_args(
        ...
        matrix_group = parser.add_argument_group(
            "Matrix Configuration", "Matrix custom settings"
        )
        ...
        # matrix custom configuration
        matrix_group.add_argument(
            f"--{prefix}prefill-urls",
            type=str,
            nargs="*",
            default=[],
            help="List of prefill worker URLs. Supports IPv4 and IPv6 addresses (use brackets for IPv6, e.g., --prefill-urls http://[::1]:8000 http://192.168.1.1:8000)",
        )
        matrix_group.add_argument(
            ...
            nargs="*",
            default=[],
            help="List of decode worker URLs. Supports IPv4 and IPv6 addresses (use brackets for IPv6, e.g., --decode-urls http://[::1]:8000 http://192.168.1.1:8000)",
        )
        matrix_group.add_argument(
            f"--{prefix}prefiller-hosts",
            type=str,
            nargs="*",
            default=[],
            help="List of prefill worker hosts. (e.g., --prefiller-hosts 192.168.1.1 192.168.1.2)",
        )
        matrix_group.add_argument(
            f"--{prefix}prefiller-ports",
            type=str,
            nargs="*",
            default=[],
            help="List of prefill worker ports. (e.g., --prefiller-ports 8001 8002)",
        )
        matrix_group.add_argument(
            f"--{prefix}decoder-hosts",
            type=str,
            nargs="*",
            default=[],
            help="List of decode worker hosts. (e.g., --decoder-hosts 192.168.1.3 192.168.1.4)",
        )
        matrix_group.add_argument(
            f"--{prefix}decoder-ports",
            type=str,
            nargs="*",
            default=[],
            help="List of decode worker ports. (e.g., --decoder-ports 8003 8004)",
        )
        matrix_group.add_argument(
            f"--{prefix}static-models",
            type=str,
            nargs="*",
            default=[],
            help="List of static models to load at startup. These models will not be managed by the router.",
        )
        matrix_group.add_argument(
            f"--{prefix}data-plane-api-keys",
            type=str,
            nargs="*",
            default=[],
            help='Data Plane Authentication. Supports environment variable import with multiple values (e.g., export DATA_PLANE_API_KEYS="sk-123,sk-456")',
        )
        ...
        backend_group.add_argument(
            ...
            choices=["vllm", "sglang", "openai", "anthropic"],
    ...
    def from_cli_args(cls, args: argparse.Namespace, use_router_prefix: bool = False) -> RouterArgs:
        ...
        # 扩展自定义的prefill_urls和decode_urls参数
        if f"{prefix}prefill_urls" in cli_args_dict:
            args_dict["prefill_urls"].extend((url, None) for url in cli_args_dict.get(f"{prefix}prefill_urls"))
        if f"{prefix}decode_urls" in cli_args_dict:
            args_dict["decode_urls"].extend(url for url in cli_args_dict.get(f"{prefix}decode_urls"))
        
        #扩展自定义的prefiller_hosts和prefiller_ports参数
        prefiller_hosts = cli_args_dict.get(f"{prefix}prefiller_hosts", [])
        prefiller_ports = cli_args_dict.get(f"{prefix}prefiller_ports", [])
        if prefiller_hosts and prefiller_ports:
            # 验证hosts和ports数量是否匹配
            if len(prefiller_hosts) != len(prefiller_ports):
                raise ValueError(
                    f"Number of prefiller hosts ({len(prefiller_hosts)}) must match "
                    f"number of prefiller ports ({len(prefiller_ports)})"
                )
            # 验证端口是否为有效数字
            for i, port in enumerate(prefiller_ports):
                if not port.isdigit():
                    raise ValueError(
                        f"prefiller_ports[{i}] = '{port}' is not a valid port number"
                    )
            args_dict["prefill_urls"].extend(
                (f"http://{host}:{port}", None)
                for host, port in zip(prefiller_hosts, prefiller_ports)
            )
        #扩展自定义的decoder_hosts和decoder_ports参数
        decoder_hosts = cli_args_dict.get(f"{prefix}decoder_hosts", [])
        decoder_ports = cli_args_dict.get(f"{prefix}decoder_ports", [])
        if decoder_hosts and decoder_ports:
            # 验证hosts和ports数量是否匹配
            if len(decoder_hosts) != len(decoder_ports):
                raise ValueError(
                    f"Number of decoder hosts ({len(decoder_hosts)}) must match "
                    f"number of decoder ports ({len(decoder_ports)})"
                )
            # 验证端口是否为有效数字且在合理范围内
            for i, port in enumerate(decoder_ports):
                if not port.isdigit():
                    raise ValueError(
                        f"decoder_ports[{i}] = '{port}' is not a valid port number"
                    )
            args_dict["decode_urls"].extend(
                f"http://{host}:{port}"
                for host, port in zip(decoder_hosts, decoder_ports)
            )
        ...
        #设置pd_disaggregation标志
        if (args_dict.get("prefill_urls") and args_dict.get("decode_urls")) or (
            args_dict.get("prefill_selector") and args_dict.get("decode_selector")
        ):
            args_dict["pd_disaggregation"] = True

        # 从环境中获取数据平面API KEY
        if os.getenv("DATA_PLANE_API_KEYS"):
            args_dict["data_plane_api_keys"] = cli_args_dict.get(f"{prefix}data_plane_api_keys", [])
            args_dict["data_plane_api_keys"].extend(os.getenv("DATA_PLANE_API_KEYS").split(","))
